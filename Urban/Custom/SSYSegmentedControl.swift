...
@IBDesignable open class SSYSegmentedControl: UIControl {
    ...
    //Kangtle
//    @IBInspectable open override var cornerRadius : CGFloat = CGFloat(0) {
    ...
    /** Font used in labels.  Defaults to system font of size 17.0, which is the
     ...
     someday it will.*/
    @IBInspectable open var font : UIFont = UIFont.systemFont(ofSize: 12.0) {
        didSet {
            for label in labels {
                label.font = font
            }
    ...
    @IBInspectable open var deselectedSegmentColor: UIColor? = UIColor.init(rgb: 0x114157) {
        didSet {
    ...
    func updateLabelsTextColor() {
        for label in self.labels {
            ...
            let newColor = isSelected ? UIColor.white : deselectedSegmentColor
            let bgColor = isSelected ? deselectedSegmentColor : UIColor.clear
